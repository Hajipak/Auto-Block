...
local bdBlockDelay = 0  -- Delay Block untuk Better Detection
    ...
    if antiFlickOn then
        ...
        task.spawn(function()
            ...
            for i = 1, count do
                ...
                -- combine lateral predictions
                local totalLateral = lateralPredict + turnLateral

                -- final predicted offset
                local predictedOffset = Vector3.new(totalLateral, 0, -forwardPredict)

                -- spawn the block part at predicted position
                local blockPart = Instance.new("Part")
                blockPart.Name = "BlockPart" .. i
                blockPart.Size = partSize
                ...
                -- position: killer's HRP + offset + forward dist
                local spawnPos = hrp.Position + hrp.CFrame.LookVector * dist + predictedOffset
                blockPart.CFrame = CFrame.new(spawnPos, hrp.Position)

                -- check if player's HRP intersects with this part
                if myRoot and isPointInsidePart(blockPart, myRoot.Position) then
                    if not blocked then
                        blocked = true
                        if not (facingCheckEnabled and not isFacing(myRoot, hrp)) then
                            task.wait(bdBlockDelay or 0) -- Delay Block tambahan untuk Better Detection
                ...
                Debris:AddItem(blockPart, lifeTime)
...
if Library then
    ...
    BDLeftGroup:AddInput("BDBlockDelay", {
        Text = "Delay Block (seconds)",
        Default = "0",
        Numeric = true,
        ClearTextOnFocus = false,
        Placeholder = "0",
        Callback = function(Value)
            bdBlockDelay = tonumber(Value) or 0
        end,
    })
    ...
    MiscRightGroup:AddButton("infinite resistence 100% real not fake trust", function()
        lp:Kick("u got banned from robl
